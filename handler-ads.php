			...
			if ($response != null && $response->success) 
			{
				function check_filling($fil, $desc) {
					if (empty($fil)) {
						$_SESSION['ad_add'] = 3;
						$_SESSION['ad_er'] = $desc;
						wp_redirect( home_url('/продать-бизнес/') );
						exit;
					}
				}

				//Проверяем заполнены ли обязательные поля
				check_filling ($_POST['post_title'], 'Введите название своего бизнеса');
				check_filling ($_POST['ale_areal'], 'Расположение бизнеса');
				...
				check_filling ($_POST['ale_cost'], 'Цена продаваемого бизнеса');
				...
				if (!empty($_POST['ale_bus_type'])) {
					add_post_meta( $post_id, 'ale_bus_type', strip_tags(trim($_POST['ale_bus_type'])) );
				}
				if (!empty($_POST['ale_category'])) {
					add_post_meta( $post_id, 'ale_category', strip_tags(trim($_POST['ale_category'])) );
				}
				if (!empty($_POST['ale_areal'])) {
					add_post_meta( $post_id, 'ale_areal', strip_tags(trim($_POST['ale_areal'])) );
				}
				...
				if (!empty($_POST['ale_datefield'])) {
					...
					$_SESSION['ad_add'] = 3;
					$_SESSION['ad_errror'] = "Вы не заполнили поле 'Описание товара'";
					wp_redirect( home_url('/продать-бизнес/') );
					exit;
				}
